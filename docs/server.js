...
const ADMIN_SECRET =
  process.env.ADMIN_SECRET || "change_this_jwt_secret_to_a_long_random_value";
...
async function appendLog(entry) {
  ...
  await writeJson(LOGS_FILE, logs.slice(0, 2000));
}
...
function validateSubmission(name, message) {
  if (!name || !message) return "Missing name or message";
  ...
  if (validator.contains(message, "http")) return "Links are not allowed";
  return null;
}
async function verifyRecaptcha(token, remoteip) {
  ...
  try {
    ...
    return j.success && (j.score === undefined || j.score >= 0.45);
  } catch (e) {
...
app.post("/api/submit", async (req, res) => {
  try {
    const { name, message, recaptchaToken } = req.body;
    const n = sanitizeString(name, 200);
    const m = sanitizeString(message, 2000);
    const err = validateSubmission(n, m);
    if (err) return res.status(400).json({ ok: false, error: err });
    if (RECAPTCHA_SECRET) {
      const ok = await verifyRecaptcha(recaptchaToken, req.ip);
      if (!ok) {
        await appendLog({ type: "spam-blocked", detail: { ip: req.ip } });
    ...
    await writeJson(SUBMISSIONS_FILE, submissions.slice(0, 10000));
    await appendLog({
      type: "submission",
      detail: { name: n, id: item.id, ip: req.ip },
    });
    return res.json({
      ok: true,
...
app.post("/api/admin/login", authLimiter, async (req, res) => {
  try {
    ...
    if (!match) {
      await appendLog({ type: "auth-fail", detail: { ip: req.ip } });
      return res.status(401).json({ ok: false, error: "Invalid password" });
    }
    const token = jwt.sign({ role: "admin" }, ADMIN_SECRET, {
      expiresIn: "6h",
    });
    await appendLog({ type: "auth-success", detail: { ip: req.ip } });
...
app.get("/api/admin/logs", requireAdmin, async (req, res) => {
  try {
    ...
    return res.json({
      ...
      submissions: submissions.slice(0, 2000),
    });
  } catch (e) {
    console.error(e);
    return res.status(500).json({ ok: false, error: "Server error" });
...
app.post("/api/admin/publish", requireAdmin, async (req, res) => {
  try {
    ...
    if (!id || publish === undefined)
      return res
        .status(400)
        .json({ ok: false, error: "Missing id or publish flag" });
    ...
    await appendLog({
      type: "publish-change",
      detail: { id, publish: !!publish, adminIp: req.ip },
    });
